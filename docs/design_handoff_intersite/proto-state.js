...
function protoReduce(state, action) {
  ...
  switch (action.type) {
    ...
    // ─── Historical backfill ────────────────────────────────────────
    // The work already happened. Collapses request+PO+delivery+settlement
    // into one shot. Creates a thread already at 'in-use' (or 'exhausted')
    // with all the lifecycle records backfilled. For group, writes
    // interSiteUsage from the captured % split so inter-site debt picks it up.
    case 'BACKFILL_THREAD': {
      const p = action.payload;
      // Ensure draft vendor/material exist in catalogs before the thread
      // renders. ProtoThreadRow looks them up by id.
      ...
      const t = {
        id,
        site: p.site || 'srinivasan',
        section: p.section || 'Historical',
        priority: 'normal',
        kind: p.kind,
        advance: false,
        ...
        delivery: {
          date: p.purchaseDate, recordedBy: 'ajith',
    ...
    case 'BACKFILL_BATCH': {
      // Bulk ingest from the AI-assisted flow.
      const rows = action.payload;
      // First: push draft vendors and materials into the catalog so the
      // rendered threads can look them up. Mirror what manual entry does.
      rows.forEach(p => {
        if (p.materialIsDraft && !M.material(p.material)) {
          M_MATERIALS.push({
            id: p.material, name: p.materialName || p.material,
