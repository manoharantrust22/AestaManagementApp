-- Migration: Emit one row per settlement_group in v_all_expenses Daily Salary
--
-- Before this change, the Daily Salary branch of v_all_expenses aggregated
-- multiple settlement_groups by (site_id, settlement_date) into a single row
-- via array_agg(sg.id)[1] + GROUP BY. That made the All Site Expenses page
-- under-count Daily Salary versus /site/payments whenever a site had more
-- than one daily settlement on the same date (multiple mestris, splits,
-- corrections). The user's mental model is one settlement event = one record;
-- this migration aligns the view with that model.
--
-- After this change Daily Salary mirrors the Contract Salary branch: one row
-- per settlement_groups.id, no GROUP BY. Counts on /site/expenses now match
-- /site/payments exactly for the same date range and site.
--
-- Other branches (Contract Salary, Advance, Excess, Unlinked Salary, Tea Shop,
-- Misc, Subcontract Direct, Material Purchases) are unchanged.
--
-- Reversible: re-run 20260506120000_split_daily_salary_excess_unlinked.sql
-- which restores the per-date aggregation.
...
-- Daily Salary settlements (CHANGED 2026-05-07: one row per settlement_group,
-- no GROUP BY. Counts now match /site/payments. Same WHERE clause as before:
-- payment_type='salary' OR NULL, has attendance link, no contract labor.)
SELECT "sg"."id",
    "sg"."site_id",
...
-- Miscellaneous expenses (with SMART CATEGORY MAPPING)
SELECT
    "me"."id",
    "me"."site_id",
    "me"."date",
...
COMMENT ON VIEW "public"."v_all_expenses" IS
  'Unified expense view. Salary settlement_groups split into five expense_types: Daily Salary (per-SG, requires daily_attendance or market_laborer_attendance link, is_archived=false; one row per settlement_group, NOT aggregated by date), Contract Salary (per-SG, requires labor_payments.is_under_contract=true), Advance (payment_type=advance), Excess (payment_type=excess), Unlinked Salary (payment_type=salary with no attendance and no labor_payment link). Counts on /site/expenses now match /site/payments exactly.';
