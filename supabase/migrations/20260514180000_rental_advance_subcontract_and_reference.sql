-- Add subcontract_id to rental_advances so advances can be linked to a trade.
-- Also update v_all_expenses to surface the subcontract and derive a reference code.
...
-- 2. Backfill from the vendor settlement on the same order (best-effort)
UPDATE rental_advances ra
SET subcontract_id = rs.subcontract_id
FROM rental_settlements rs
WHERE ra.rental_order_id = rs.rental_order_id
  AND rs.subcontract_id IS NOT NULL
  AND ra.subcontract_id IS NULL;

-- 3. Rebuild view so advances show correct trade + reference code
...
UNION ALL
 SELECT sg.id,
  ...
  WHERE sg.is_cancelled = false AND sg.is_archived = false AND COALESCE(sg.payment_type, 'salary'::text) = 'salary'::text AND NOT (EXISTS ( SELECT 1
           FROM labor_payments lp
          WHERE lp.settlement_group_id = sg.id AND lp.is_under_contract = true)) AND ((EXISTS ( SELECT 1
           FROM daily_attendance da
          WHERE da.settlement_group_id = sg.id AND da.is_archived = false)) OR (EXISTS ( SELECT 1
...
UNION ALL
 SELECT sg.id,
    ...
    sg.total_amount AS amount,
        CASE
            WHEN sg.notes IS NOT NULL AND sg.notes <> ''::text THEN (('Excess payment ('::text || sg.laborer_count) || ' laborers) - '::text) || sg.notes
...
UNION ALL
 SELECT sg.id,
    ...
    sg.total_amount AS amount,
        CASE
            WHEN sg.notes IS NOT NULL AND sg.notes <> ''::text THEN (('Unlinked salary ('::text || sg.laborer_count) || ' laborers) - '::text) || sg.notes
  ...
  WHERE sg.is_cancelled = false AND sg.is_archived = false AND COALESCE(sg.payment_type, 'salary'::text) = 'salary'::text AND NOT (EXISTS ( SELECT 1
           FROM labor_payments lp
          WHERE lp.settlement_group_id = sg.id)) AND NOT (EXISTS ( SELECT 1
           FROM daily_attendance da
          WHERE da.settlement_group_id = sg.id)) AND NOT (EXISTS ( SELECT 1
           FROM market_laborer_attendance ma
          WHERE ma.settlement_group_id = sg.id))
UNION ALL
 SELECT ts.id,
    ...
    'Tea & Snacks'::text AS expense_type,
        CASE
            ...
            WHEN ts.site_engineer_transaction_id IS NOT NULL THEN true
            ELSE true
        END AS is_cleared,
        CASE
            WHEN ts.payer_type::text = 'company_direct'::text THEN ts.payment_date
            WHEN ts.site_engineer_transaction_id IS NOT NULL THEN ( SELECT site_engineer_transactions.transaction_date
               FROM site_engineer_transactions
              WHERE site_engineer_transactions.id = ts.site_engineer_transaction_id)
...
UNION ALL
 SELECT me.id,
    ...
    'miscellaneous'::text AS module,
        CASE
            WHEN ec.name::text = 'Daily Labor Settlement'::text THEN 'Daily Salary'::text
        ...
        CASE
            ...
            WHEN me.engineer_transaction_id IS NOT NULL THEN true
            ELSE true
        END AS is_cleared,
        CASE
            WHEN me.payer_type = 'company_direct'::text THEN me.date
            WHEN me.engineer_transaction_id IS NOT NULL THEN ( SELECT site_engineer_transactions.transaction_date
               FROM site_engineer_transactions
    ...
    NULL::uuid AS site_payer_id,
        CASE
            WHEN me.payer_type = 'site_engineer'::text THEN COALESCE(( SELECT users.name
               FROM users
...
UNION ALL
 SELECT sp.id,
    ...
    sp.amount,
        CASE
            WHEN sp.comments IS NOT NULL AND sp.comments <> ''::text THEN (('Contract Payment - '::text || sc.title::text) || ' - '::text) || sp.comments
            ELSE 'Contract Payment - '::text || sc.title::text
        END AS description,
    ( SELECT expense_categories.id
           FROM expense_categories
    ...
    'Direct Payment'::text AS expense_type,
        CASE
            ...
            WHEN sp.site_engineer_transaction_id IS NOT NULL THEN true
            ELSE true
        END AS is_cleared,
        CASE
            WHEN sp.payment_channel = ANY (ARRAY['company_direct_online'::text, 'mesthri_at_office'::text]) THEN sp.payment_date
            WHEN sp.site_engineer_transaction_id IS NOT NULL THEN ( SELECT site_engineer_transactions.transaction_date
               FROM site_engineer_transactions
    ...
    NULL::uuid AS site_payer_id,
        CASE
            ...
            WHEN sp.payment_channel = 'via_site_engineer'::text THEN COALESCE(( SELECT users.name
               FROM users
...
UNION ALL
 SELECT mpe.id,
    ...
    COALESCE(mpe.amount_paid, mpe.total_amount) AS amount,
        CASE
            WHEN mpe.notes IS NOT NULL AND mpe.notes <> ''::text THEN (('Material Purchase - '::text || COALESCE(mpe.vendor_name, 'Unknown'::text)) || ' - '::text) || mpe.notes
            ELSE 'Material Purchase - '::text || COALESCE(mpe.vendor_name, 'Unknown'::text)
        END AS description,
    ( SELECT expense_categories.id
           FROM expense_categories
    ...
    NULL::uuid AS settlement_group_id,
    mpe.engineer_transaction_id,
    'material_purchase'::text AS source_type,
    mpe.id AS source_id,
    mpe.created_at,
    false AS is_deleted
   FROM material_purchase_expenses mpe
  WHERE (mpe.is_paid = true OR mpe.settlement_date IS NOT NULL) AND (mpe.purchase_type IS DISTINCT FROM 'group_stock'::text OR mpe.settlement_reference IS NOT NULL)
UNION ALL
-- Rental settlements
 SELECT rs.id,
    ro.site_id,
    ...
    COALESCE(rs.negotiated_final_amount, rs.balance_amount + COALESCE(rs.total_advance_paid, 0)) AS amount,
        CASE
            WHEN rs.notes IS NOT NULL AND rs.notes <> ''::text THEN ('Rental - '::text || COALESCE(v.shop_name, v.name)) || ' - '::text || rs.notes
            ELSE 'Rental - '::text || COALESCE(v.shop_name, v.name)
        END AS description,
    ( SELECT expense_categories.id
           FROM expense_categories
    ...
    'Machinery'::text AS expense_type,
        CASE
            WHEN rs.payment_channel = 'direct'::text THEN true
            WHEN rs.engineer_transaction_id IS NOT NULL THEN true
            ELSE true
        END AS is_cleared,
        CASE
            WHEN rs.payment_channel = 'direct'::text THEN rs.settlement_date
            WHEN rs.engineer_transaction_id IS NOT NULL THEN ( SELECT site_engineer_transactions.transaction_date
               FROM site_engineer_transactions
...
UNION ALL
-- Rental advances — subcontract_id and reference derived from order number
 SELECT ra.id,
    ro.site_id,
    ra.advance_date AS date,
    ra.created_at::date AS recorded_date,
    ra.amount,
        CASE
            WHEN ra.notes IS NOT NULL AND ra.notes <> ''::text
                THEN 'Rental Advance - '::text || COALESCE(v.shop_name, v.name) || ' - '::text || ra.notes
            ELSE 'Rental Advance - '::text || COALESCE(v.shop_name, v.name)
        END AS description,
    ( SELECT expense_categories.id
           FROM expense_categories
          WHERE expense_categories.name::text = 'Rental'::text AND expense_categories.module = 'machinery'
    ...
    'Machinery'::text AS expense_type,
    true AS is_cleared,
    ra.advance_date AS cleared_date,
    ra.subcontract_id AS contract_id,
    sc.title AS subcontract_title,
    NULL::uuid AS site_payer_id,
        CASE
            WHEN ra.payer_source IS NULL THEN 'Own Money'::text
            ...
            ELSE COALESCE(ra.payer_name, 'Own Money'::text)
        END AS payer_name,
    ra.payment_mode,
    COALESCE(v.shop_name, v.name) AS vendor_name,
    ra.proof_url AS receipt_url,
    ra.created_by AS paid_by,
    NULL::text AS entered_by,
    ra.created_by AS entered_by_user_id,
    (ro.rental_order_number || '/ADV') AS settlement_reference,
    NULL::uuid AS settlement_group_id,
    ra.engineer_transaction_id,
    'rental_advance'::text AS source_type,
    ra.id AS source_id,
    ra.created_at,
    false AS is_deleted
...
GRANT SELECT ON v_all_expenses TO authenticated, anon;

COMMENT ON VIEW v_all_expenses IS
  'Unified expenses view. v2.3 (2026-05-14): rental_advances get subcontract_id + order-number reference code.';
