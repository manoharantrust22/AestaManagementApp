...
-- bearing branch surfaces its row's split.
--
-- Of the 3 named branches, only 2 actually carry a per-row
-- payer_source_split column (tea_shop_settlements + material_purchase_expenses
-- — both wired in the 20260523140000 foundation). subcontract_payments has
-- no payer_source_split column on its source table, so its branch keeps
-- projecting NULL::jsonb and continues to fall back to the Phase 4
-- engineer_wallet_spend_allocations rollup at the trailing SELECT.
