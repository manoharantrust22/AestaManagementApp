-- Slice 6 — scope the Salary Settlement KPI-strip aggregates to one in-house
-- trade contract (a subcontract_id) for the trade-workspace settlement screen.
--
-- get_payment_summary returns server-side SUM/COUNT aggregates (no per-row
-- breakdown), so it cannot be filtered client-side — it needs a scope param.
-- Adds an optional p_subcontract_id (DEFAULT NULL) and filters the three source
-- CTEs (pending_da / pending_ma / paid_groups) by it. When p_subcontract_id IS
-- NULL (Civil / unscoped) the `IS NULL OR …` predicate is a no-op, so every
-- existing caller gets byte-for-byte identical aggregates. Mirrors the
-- p_subcontract_id pattern in get_salary_waterfall.
--
-- Body reproduced verbatim from the live definition
-- (20260504110200_audit_mode_get_payment_summary.sql); only the new parameter
-- and three WHERE predicates are added. The parameter list changes, so the
-- prior signatures are DROPped first and EXECUTE re-granted.
...
'Salary Settlement KPI strip aggregates. p_period (all/legacy/current) lets callers split the strip into Legacy + Current cards when site is in auditing state. p_subcontract_id (NULL = whole site) scopes the strip to one in-house trade contract for the trade workspace. is_archived=false honors Mode B reconcile.';
