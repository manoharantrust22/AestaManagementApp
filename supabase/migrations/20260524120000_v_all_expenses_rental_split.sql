...
-- rental domain branches (rental_settlements + rental_advances). The
-- settlement_groups branches were wired in Phase 1 (20260523140200);
-- misc_expenses in Phase 2 (20260523150000); this finishes rentals.
-- The 3 remaining branches (tea_shop_settlements, subcontract_payments,
-- material_purchase_expenses) stay on NULL — their domain dialogs still
-- write the legacy single-source shape.
--
-- Note: filename uses 20260524120000 rather than 20260524100000 from the
-- plan because 20260524100000 is already taken by spot_purchase_schema.sql
-- and 20260524110000 by daily_peek_spot_purchases.sql.
...
COMMENT ON COLUMN v_all_expenses.payer_source_split IS
  'Canonical JSONB array [{"source":..., "amount":...}, ...] of the row''s payer-source breakdown. Resolution: (1) the row''s per-row split column when set (Phase 1 wires this for settlement_groups via sg.payer_source_split; Phase 2 wires misc_expenses via me.payer_source_split; Phase 3 wires rental_settlements via rs.payer_source_split and rental_advances via ra.payer_source_split; the remaining 3 branches — tea_shop_settlements, subcontract_payments, material_purchase_expenses — stay on NULL until their dialogs adopt the split shape); (2) else, for any row whose engineer_transaction_id points at a wallet spend, the Phase 4 fallback aggregates engineer_wallet_spend_allocations into the same array shape; (3) else NULL. Shape change vs. the prior Phase 4 view: the wallet-allocation rollup now emits the canonical array instead of the legacy {source: amount} object, and rows with no split at all return NULL instead of ''{}''::jsonb.';
