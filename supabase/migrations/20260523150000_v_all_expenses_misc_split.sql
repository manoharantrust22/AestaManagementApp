...
COMMENT ON COLUMN v_all_expenses.payer_source_split IS
  'Canonical JSONB array [{"source":..., "amount":...}, ...] of the row''s payer-source breakdown. Resolution: (1) the row''s per-row split column when set (Phase 1 wires this for settlement_groups via sg.payer_source_split; Phase 2 wires misc_expenses via me.payer_source_split; other domains land in Phase 3); (2) else, for any row whose engineer_transaction_id points at a wallet spend, the Phase 4 fallback aggregates engineer_wallet_spend_allocations into the same array shape; (3) else NULL. Shape change vs. the prior Phase 4 view: the wallet-allocation rollup now emits the canonical array instead of the legacy {source: amount} object, and rows with no split at all return NULL instead of ''{}''::jsonb.';
