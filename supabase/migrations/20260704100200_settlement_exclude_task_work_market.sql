-- Exclude task-work-package MARKET attendance from the daily SALARY SETTLEMENT.
--
-- 20260702130000 removed daily_attendance rows tagged with a task_work_package_id
-- from the per-day salary waterfall (they are paid through the contract instead).
-- Market laborers can now carry the same tag (20260704100000) but were still
-- summed into the daily/market pending & paid settlement — so an ad-hoc crew
-- assigned to a fixed-price package would wrongly appear as per-day settleable and
-- could double-pay against the package's price. Apply the same rule to market rows.
--
-- Rule (parallel to the daily side): a market_laborer_attendance row leaves the
-- salary waterfall when task_work_package_id IS NOT NULL.
--
-- OVERVIEW figures stay all-inclusive, exactly as the daily migration did:
-- market_amount / market_count (and thus total_salary / total_expense) are
-- UNCHANGED. Only the paid/pending market fields (get_attendance_summary) and the
-- pending_ma CTE (get_payments_ledger) gain the task-work exclusion.
  ...
  ma_totals AS (
    SELECT
      ...
      COALESCE(SUM(total_cost) FILTER (
        WHERE is_paid AND task_work_package_id IS NULL AND NOT EXISTS (
      ...
      COALESCE(SUM(total_cost) FILTER (
        WHERE NOT is_paid AND task_work_package_id IS NULL AND NOT EXISTS (
          SELECT 1 FROM da WHERE da.date = ma.date
        )
...
AS $function$
  ...
  pending_ma AS (
    ...
    WHERE m.site_id = p_site_id AND m.is_paid = false AND m.settlement_group_id IS NULL
      AND m.task_work_package_id IS NULL
