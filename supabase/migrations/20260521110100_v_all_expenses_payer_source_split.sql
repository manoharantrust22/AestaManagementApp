-- Phase 4 of the wallet payer-source attribution feature.
--
-- Adds a single new column `payer_source_split` (JSONB) to v_all_expenses.
-- For each row whose engineer_transaction_id points at a wallet spend, this
-- column aggregates the allocations table into { "amma_money": 375, ... } so
-- consumers can answer "of this expense's ₹X, how much came from each source"
-- without a second query.
--
-- Rows that aren't wallet-funded (engineer_transaction_id IS NULL or no
-- allocations exist yet) get '{}'::jsonb. Reports fall back to the row's
-- own legacy payer_name column in that case.
--
-- The view body is identical to the previous definition (captured 2026-05-21);
-- only the trailing `payer_source_split` column is new. CREATE OR REPLACE VIEW
-- accepts column-append without dropping dependents.
...
SELECT
  ...
  COALESCE(
    (
      SELECT jsonb_object_agg(payer_source, total)
      FROM (
...
COMMENT ON COLUMN v_all_expenses.payer_source_split IS
  'JSONB { payer_source: amount } breakdown of wallet-funded rows. Derived from engineer_wallet_spend_allocations via the row.engineer_transaction_id link. Empty object for rows that are not wallet-funded. Phase 4 of the wallet payer-source attribution feature.';
