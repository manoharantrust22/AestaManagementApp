-- Fix: material_purchase_expenses.created_by → auth.users(id) so must use auth.uid().
-- while price_history.recorded_by → public.users(id) so must use v_public_user_id.
-- Previous migration (20260510140000) incorrectly passed v_public_user_id for
-- material_purchase_expenses.created_by, breaking site-expense mode ingestion.
--
-- Summary of user ID conventions in this schema:
--   material_purchase_expenses.created_by  → auth.users(id)    use auth.uid()
--   price_history.recorded_by              → public.users(id)  use v_public_user_id
...
BEGIN
  ...
  -- price_history.recorded_by FKs to public.users(id), not auth.users(id).
  -- Resolve the public.users row from the caller's auth session (nullable — no error if missing).
  ...
  IF NOT v_catalog_only THEN
    ...
    -- created_by FKs to auth.users(id) — use auth.uid() directly
  ...
  LOOP
    ...
    -- recorded_by FKs to public.users(id) — use v_public_user_id (nullable)
