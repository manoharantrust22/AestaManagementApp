-- Fix ingest_purchase_atomic to support catalog-only mode (site_id = null).
--
-- When site_id is null the caller has the "Also record as site expense" toggle
-- OFF (company-flow default). The RPC should still:
--   • resolve / create the vendor
--   • resolve / create each material + category + brand in the catalog
--   • upsert vendor_inventory with current prices
--   • insert price_history rows (source = 'bill')
-- …but SKIP:
--   • generating a ref_code
--   • inserting into material_purchase_expenses
--   • inserting into material_purchase_expense_items
-- Return: purchase_id = null, ref_code = null, expense_created = false.
