-- Surface the material→subcontract link in v_all_expenses.
--
-- Re-emits the full definition from 20260601100000_v_all_expenses_material_detail.sql.
-- The ONLY change is in the material_purchase branch: contract_id /
-- subcontract_title now come from the new material_purchase_expenses.subcontract_id
-- (via LEFT JOIN subcontracts) instead of hard-coded NULL. The WHERE clause is
-- unchanged, so unlinked paid materials still appear on /site/expenses (their
-- contract_id is just NULL and aggregations-by-contract ignore them).
--
-- This single change feeds calculateSubcontractTotals() (which reads
-- v_all_expenses by contract_id), so linked, paid materials now roll into a
-- subcontract's total on the Subcontracts page. Additive / non-destructive.
--
-- DROP + recreate (not CREATE OR REPLACE): the live view's `amount` column
-- carries a numeric(12,2) typmod, and CREATE OR REPLACE cannot change a column's
-- resolved type (Postgres 42P16). Nothing depends on v_all_expenses (verified via
-- pg_depend), so dropping it is safe; the grants below restore access. No
-- security_invoker option to preserve (reloptions is null on the live view).
...
COMMENT ON COLUMN v_all_expenses.contract_id IS
  'Subcontract the row is linked to (NULL = unlinked). Material-purchase rows now expose material_purchase_expenses.subcontract_id here, so linked, paid materials roll into a subcontract''s spend via calculateSubcontractTotals().';
