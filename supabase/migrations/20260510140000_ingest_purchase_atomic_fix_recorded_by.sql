-- Fix ingest_purchase_atomic: resolve public.users.id from auth.uid()
-- before inserting into price_history / material_purchase_expenses.
--
-- public.users.id ≠ auth.users.id in this schema. The auth UUID lives in
-- public.users.auth_id. Passing auth.uid() directly to any column that has
-- a FK to public.users(id) causes a FK-constraint violation for every user.
...
DECLARE
  ...
  v_public_user_id UUID;   -- public.users.id resolved from auth_id
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'Authentication required';
  END IF;

  -- Resolve the public.users.id that corresponds to the caller's auth session.
  -- public.users.auth_id = auth.users.id; public.users.id is its own UUID.
  ...
  IF v_public_user_id IS NULL THEN
    RAISE EXCEPTION 'User profile not found for auth session';
  END IF;
  ...
  -- catalog_only = true when site_id is null (company-flow, toggle off).
  v_catalog_only := (v_site_id IS NULL);
  ...
  END IF;
  IF NOT v_catalog_only AND v_purchase_type NOT IN ('own_site', 'group_stock') THEN
    RAISE EXCEPTION 'purchase_type must be own_site or group_stock';
  END IF;

  -- 1. Resolve vendor (always required)
  v_vendor_id := _ai_ingest_resolve_vendor(p_payload->'vendor');
  SELECT name INTO v_vendor_name FROM vendors WHERE id = v_vendor_id;

  -- 2. Generate ref_code + insert purchase header (skipped in catalog-only mode)
  IF NOT v_catalog_only THEN
    ...
    INSERT INTO material_purchase_expenses (
      site_id, ref_code, purchase_type, vendor_id, vendor_name,
      purchase_date, total_amount, transport_cost, payment_mode,
      payment_reference, bill_url, status, notes, created_by
    )
    VALUES (
      v_site_id, v_ref_code, v_purchase_type, v_vendor_id, v_vendor_name,
      v_purchase_date, v_total_amount, v_transport_cost, v_payment_mode,
      v_invoice_no, v_bill_url, 'recorded',
  ...
  LOOP
    ...
    -- 3d. Insert expense item (skipped in catalog-only mode)
    IF NOT v_catalog_only THEN
    ...
    -- 3e. Price history (always — catalog-only still tracks prices)
    ...
    VALUES (
      ...
      v_bill_url, v_invoice_no, v_purchase_date, v_public_user_id
    );

    -- 3f. Upsert vendor_inventory (always)
  ...
  RETURN jsonb_build_object(
    ...
    'expense_created', NOT v_catalog_only
  );
END;
$body$;
