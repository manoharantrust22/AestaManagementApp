-- Restore rental_settlement union in v_all_expenses
--
-- Migration 20260509120500 rebuilt v_all_expenses for wallet v2 compatibility but
-- accidentally dropped the rental_settlements UNION ALL clause. This migration
-- adds it back with v2-compatible expressions (transaction_date instead of the
-- now-gone confirmed_at/is_settled columns).
...
UNION ALL
-- Rental settlements: restored here after being dropped by 20260509120500.
-- v2 wallet: engineer_transaction_id IS NOT NULL means cleared (no is_settled check).
-- cleared_date reads transaction_date from the linked spend row (not confirmed_at).
 SELECT rs.id,
    ro.site_id,
...
COMMENT ON VIEW v_all_expenses IS
  'Unified expenses view across labor, material, tea shop, misc, subcontract, and rental domains. v2 (2026-05-09): wallet-funded entries are cleared the moment the spend ledger row exists; cleared_date reads transaction_date from site_engineer_transactions. v2.1 (2026-05-13): rental_settlement UNION restored.';
