-- reverse_delivery: support per-installment reversal on group/advance POs + a
-- short-circuit for empty (0-item) deliveries.
--
-- The original refused to reverse a single delivery whenever its group PO had
-- ANY other deliveries ("reverse the whole PO instead"). But an advance/group PO
-- is delivered in installments by design, and a user legitimately needs to undo
-- ONE mis-entered installment. The stock a delivery adds lives in per-site
-- `stock_inventory` batch rows and is reversed precisely via this delivery's
-- `stock_transactions` (reference_type='delivery', reference_id=delivery_id) —
-- which step 4 already does. Group `group_stock_inventory`/`group_stock_transactions`
-- are NOT written per-delivery (reference_type is null and most group batches
-- have no group_stock_inventory row at all), so the full batch cleanup must keep
-- running ONLY when this is the sole delivery (already the condition). Hence:
--   1. drop the "group PO has other deliveries" guard, and
--   2. add an empty-delivery short-circuit (nothing to reverse -> just delete it,
--      bypassing the usage/settlement guards which key off the batch, not the row).
-- The genuine safety guards stay: usage logged, inter-site settled, own-pool would
-- go negative, last-delivery-and-vendor-already-paid.
